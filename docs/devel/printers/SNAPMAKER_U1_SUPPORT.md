...
- **Remote screen ("gui" webcam)** -- Wired up via the U1 platform hook (`config/platform/hooks-snapmaker-u1.sh`): `start_remote_screen` launches the firmware's `fb-http` tool on every HelixScreen start (so it survives the overlay boot-glob trap for free — the hook already runs on every boot), gated on the PAXX `web remote_screen` toggle. It prefers **fb-http's DRM backend** (`--backend drm --drm-device /dev/dri/card0`), which captures the buffer HelixScreen actually renders to — so the remote view matches the panel with **no framebuffer mirroring**, sidestepping the `/dev/fb0` staleness gap entirely (the original blocker; see Future Work for the alternative). Because fb-http ships in two flag-incompatible flavors (current PAXX Extended Firmware has the DRM backend; the mainline `paxx12/screen-apps` build exposes only `--port/--bind/--fb/--touch/--html-dir` and reads `/dev/fb0`), the hook **probes the installed tool** (`_remote_screen_backend_args`) and only passes the DRM flags when it advertises `--backend` — otherwise fb-http falls back to its fbdev default, so an older build is never crashed with unrecognized arguments. nginx `/screen/` route (`/etc/nginx/fluidd.d/remote-screen.conf`) and the `[webcam gui]` Moonraker slot are provided by the firmware.
...
**Implemented** via the U1 platform hook's `start_remote_screen`/`stop_remote_screen` (see the Known Issues entry above). HelixScreen starts the firmware's `fb-http` tool with its **DRM backend**, which captures `/dev/dri/card0` — the buffer HelixScreen renders to — directly. This needs no in-process server, no encoder, and crucially **no framebuffer mirroring**: the remote view is HelixScreen's actual output, live, so both original gaps (daemon not running + `/dev/fb0` staleness) are closed at once. It rides HelixScreen's existing per-boot start path, so it needs no separate init script (which would fall foul of the overlay boot-glob trap).

**Alternative — the "fb0 mailbox"** (bench-tested 2026-06-14, retained for firmware whose `fb-http` lacks the DRM backend and can only read `/dev/fb0`): reuse the same daemon but keep `/dev/fb0` current from the app. fb0 is a separate buffer decoupled from our DRM scanout (proven: writing magenta to fb0 was served by the daemon while the panel stayed on HelixScreen). Mirror frames into fb0 in the existing flush hook (`DisplayManager::install_color_transform_hook()`, `src/application/display_manager.cpp`) — open `/dev/fb0` once, mmap `MAP_SHARED|PROT_WRITE`, copy dirty regions as **BGRA**, stride 1920 (480×320×4), throttled to a few fps; one main-thread memcpy, no encode, post-rotation pixels. The DRM-capture path above makes this unnecessary where the DRM backend is available.

**Bonus — remote control:** `fb-http` also injects touch to `/dev/input/event0` via its `/touch` endpoint, so this gets remote *control* nearly for free — pending a check that HelixScreen reads `event0` (separate follow-up). PNG snapshot is poll-based, not a continuous MJPEG stream.
