...
- **Remote screen ("gui" webcam)** -- Wired up via the U1 platform hook (`config/platform/hooks-snapmaker-u1.sh`): `start_remote_screen` launches the firmware's `fb-http` tool on every HelixScreen start (so it survives the overlay boot-glob trap for free — the hook already runs on every boot), gated on the PAXX `web remote_screen` toggle. It uses **fb-http's DRM backend** (`--backend drm --drm-device /dev/dri/card0`), which captures the buffer HelixScreen actually renders to — so the remote view matches the panel with **no framebuffer mirroring**, sidestepping the `/dev/fb0` staleness gap entirely (the original blocker; see Future Work for the alternative). Requires an `fb-http` build with the DRM backend (present on PAXX Extended Firmware). nginx `/screen/` route (`/etc/nginx/fluidd.d/remote-screen.conf`) and the `[webcam gui]` Moonraker slot are provided by the firmware.
