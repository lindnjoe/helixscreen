...
start_remote_screen() {
    ...
    # --drm-wait lets fb-http wait for the DRM device to be ready, so it is safe
    # to launch here (before helix-screen becomes DRM master); capture is
    # read-only and does not contend for DRM master.
    #
    ...
    # starves Klipper — this is the exact failure mode that filled tmpfs with
    # 498 MB of stray output on this platform (see docs/devel/LOGGING.md and the
    # LOGFILE tmpfs guard in config/helixscreen.init). We don't need fb-http's
    # request log for HelixScreen diagnostics, so drop it rather than risk the OOM.
    start-stop-daemon -S -b -m -p "$HELIX_REMOTE_SCREEN_PID" -x /bin/sh -- -c \
        "exec /usr/bin/python3 $HELIX_FB_HTTP --bind 127.0.0.1 --port 8092 --backend drm --drm-device /dev/dri/card0 --drm-wait 60 --html-dir $HELIX_FB_HTTP_HTML >/dev/null 2>&1"
}
